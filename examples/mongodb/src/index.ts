...
import { AdaEmbeddings } from '../../../src/index.js';

const llmApplication = await new RAGApplicationBuilder()
    .setEmbeddingModel(new AdaEmbeddings())
    .setSearchResultCount(30)
    .setVectorDb(new MongoDBAtlas({ connectionString: "mongodb+srv://ashwin:pswd123ash@bfsi-demo.2wqno.mongodb.net/?retryWrites=true&w=majority", dbName: 'sample_mflix', collectionName: 'embedded_movies', textKey: 'title'}))
    .build();

// await llmApplication.addLoader(new WebLoader({ url: 'https://en.wikipedia.org/wiki/Tesla,_Inc.' }));

console.log((await llmApplication.query('Name a movie about baseball which features Clint Eastwood?')).result);
