...
describe("SnootyDataSource", () => {
  describe("makeSnootyDataSource()", () => {
    ...
    baseMock.get(`/projects/${sourceConfig.name}/${sourceConfig.currentBranch}/documents`).reply(200, () => {
      return fs.createReadStream(Path.resolve(__dirname, "../test/snooty_sample_data.txt")
    });
    baseMock.get("/projects").reply(200, sampleSnootyMetadata)
    it("successfully loads pages", async () => {
      // TODO: fix typescript typing
      const source = await makeSnootyDataSource(sourceConfig);

      const pages = await source.fetchPages();
      expect(pages.length).toBe(12);
      // TODO: generate the body with snootyAstToMd()
