...
const mockPromotions = [
  {
    ...
    title: "Skill Tile",
    description:
      "Want to learn more? Take the [Skill Title](https://example.com/) skill!",
    url: "https://example.com/",
  },
];

const mockClassifySkill: SkillClassiferFunction = async () => {
  return mockPromotions[0];
};
...
const mockStreamConfig = {
  ...
  onPromotionLink: jest.fn().mockImplementation((args: any) => {
    args.dataStreamer.streamData({
      type: "promotion",
      data: args.promotion,
    });
...
function expectSuccessfulResult(result: GenerateResponseReturnValue) {
  ...
  expect((result.messages[5] as AssistantMessage).promotions).toEqual(
    mockPromotions
  );
}
