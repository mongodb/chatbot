...
async function main() {
    ...
    snooty: {
      ...
      filter: (page) =>
        ...
        (page.url.includes("atlas/security") ||
          page.url.includes("atlas/atlas-resource-policies") ||
          page.url.includes("oidc") ||
          page.url.includes("atlas/setup-cluster-security")),
    },
    maxConcurrency: 20,
  ...
  const pathOut = path.join(maxxPromptsDir, "atlasSecurity.md");
