...
export function makeChatCompletionRoute({
  conversations,
  llm,
  generateUserPrompt,
  ...
  return async (
    ...
    try {
      ...
      const assistantResponse = {
        ...tracedResponseMessages[tracedResponseMessages.length - 1],
        id: new ObjectId(),
        createdAt: new Date(),
      };

      assert(assistantResponse !== undefined, "No assistant message found");
      ...
      if (!shouldStream) {
        ...
        await updateTraceIfExists({
          ...
          assistantResponseMessageId: assistantResponse.id,
        });
      }
    } catch (error) {
