...
describe("POST /conversations/messages/:messageId/comment", () => {
  const endpointUrl =
    DEFAULT_API_PREFIX + "/conversations/messages/:messageId/comment";
  let app: Express;
  ...
  it("Should return 400 for invalid message ID", async () => {
    const response = await request(app)
      .post(`${DEFAULT_API_PREFIX}/conversations/messages/123/comment`)
      .set("X-FORWARDED-FOR", ipAddress)
      .set("Origin", origin)
...
describe("POST /conversations/:conversationId/messages/:messageId/comment (deprecated)", () => {
  const endpointUrl =
    DEFAULT_API_PREFIX +
