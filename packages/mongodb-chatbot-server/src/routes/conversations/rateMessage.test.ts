...
import { makeRateMessageRoute, makeRateMessageRouteV2 } from "./rateMessage";
...
describe("POST /conversations/messages/:messageId/rating", () => {
  const endpointUrl =
    DEFAULT_API_PREFIX + "/conversations/messages/:messageId/rating";
  let app: Express;
  ...
  beforeAll(async () => {
    ...
    app
      .post(endpointUrl, makeRateMessageRouteV2({ conversations }))
    ...
    testEndpointUrl = endpointUrl.replace(":messageId", String(testMsg.id));
  });
  ...
  test("Should return 400 for invalid message ID", async () => {
    const response = await request(app)
      .post(`${DEFAULT_API_PREFIX}/conversations/messages/123/rating`)
      .set("X-FORWARDED-FOR", ipAddress)
      .set("Origin", origin)
      .send({ rating: true });
...
describe("POST /conversations/:conversationId/messages/:messageId/rating (deprecated)", () => {
  const endpointUrl =
    DEFAULT_API_PREFIX +
