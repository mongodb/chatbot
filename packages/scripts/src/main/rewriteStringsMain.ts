...
import { createAzure, generateText } from "mongodb-rag-core/aiSdk";
...
function isApiVersionNotSupportedError(err: unknown): boolean {
  try {
    ...
    const message: string | undefined =
      anyErr?.data?.error?.message ?? anyErr?.message;
    const responseBody: string | undefined = anyErr?.responseBody;
    return (
      (typeof message === "string" &&
        message.toLowerCase().includes("api version not supported")) ||
      (typeof responseBody === "string" &&
        responseBody.toLowerCase().includes("api version not supported"))
    );
  } catch {
    return false;
  }
...
async function main() {
  ...
  const buildProvider = (version: string) =>
    createAzure({ apiKey, resourceName, apiVersion: version });
  const buildChatModel = (version: string) =>
    buildProvider(version)(deployment);
  const buildResponsesModel = (version: string) =>
    buildProvider(version).responses(deployment);

  // Prefer Responses API when available; fall back to Chat Completions
  let model = buildResponsesModel(apiVersion);

  const rewritten: { input: string; output: string }[] = [];
  for (const input of INPUTS) {
    // Sequential for determinism and rate limiting; parallelize if needed.
    try {
      const output = await rewriteOne(model, input);
      rewritten.push({ input, output });
      continue;
    } catch (err) {
      ...
      let retried = false;
      for (const v of fallbackApiVersions) {
        try {
          // eslint-disable-next-line no-console
          console.warn(`Retrying with Responses API api-version='${v}'`);
          model = buildResponsesModel(v);
          const output = await rewriteOne(model, input);
          rewritten.push({ input, output });
          retried = true;
          break;
        } catch (retryErr) {
          if (!isApiVersionNotSupportedError(retryErr)) {
            throw retryErr;
          }
        }
        try {
          // eslint-disable-next-line no-console
          console.warn(
            `Responses API not supported for version '${v}', retrying with Chat Completions`
          );
          model = buildChatModel(v);
          const output = await rewriteOne(model, input);
          rewritten.push({ input, output });
          retried = true;
          break;
      ...
      if (!retried) {
        throw new Error(
          `Azure OpenAI API version is not supported. Tried: ${[
            apiVersion,
            ...fallbackApiVersions,
          ].join(", ")}`
        );
