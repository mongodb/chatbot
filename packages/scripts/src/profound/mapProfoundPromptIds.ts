...
const env = getEnv({
  ...
  optional: {
    MERCURY_CASES_COLLECTION_NAME: "llm_cases_new",
  },
});
...
export async function main(args: { outputDir?: string } = {}) {
  ...
  try {
    ...
    const llmCasesCollection = db.collection<LlmCase>(env.MERCURY_CASES_COLLECTION_NAME);
    ...
    console.log(`Found ${llmCases.length} documents in llm_cases_new collection`);
    ...
    console.log(`Created map with ${promptToPromptIdMap.size} unique prompt->prompt_id mappings`);
    
    // Step 4: Match llm_cases_new documents to Profound prompts
    const successfulMatches: Array<{ _id: ObjectId; profoundPromptId: string }> = [];
    ...
    if (successfulMatches.length > 0) {
      ...
      console.log(`Successfully updated ${updateResult.modifiedCount} documents`);
      
      if (updateResult.modifiedCount !== successfulMatches.length) {
        console.warn(`Warning: Expected to update ${successfulMatches.length} documents but only updated ${updateResult.modifiedCount}`);
      }
    }
    ...
    if (args.outputDir && failures.length > 0) {
      ...
      const failuresFile = path.join(outputDir, `profound-mapping-failures-${new Date().toISOString().split('T')[0]}.json`);
...
if (require.main === module) {
  const outputDir = process.argv[2] || path.join(os.homedir(), "Desktop/mercury-results/profound-mapping");
